...
/**
 * Opens the photo library and returns the picked image without uploading
 * anywhere — for flows like club creation where there's no club id (and so
 * no storage path) to upload to yet. Pair with `uploadClubAvatarAsset` once
 * the club exists. Returns null if the user cancels the picker.
 */
export async function pickImageAsset(): Promise<PickedImageAsset | null> {
  const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
  if (!permission.granted) {
    throw new AvatarError('Photo library permission denied — enable it in Settings to set a photo.');
  }
  ...
  const ext = asset.uri.split('.').pop()?.toLowerCase() === 'png' ? 'png' : 'jpg';
  return { uri: asset.uri, base64: asset.base64, ext };
}

/**
 * Uploads a previously-picked image under `clubs/{clubId}/` — storage RLS
 * there checks the uploader is an active admin/owner of that club rather
 * than matching a user-id folder.
 */
export async function uploadClubAvatarAsset(clubId: string, asset: PickedImageAsset): Promise<string> {
  const contentType = asset.ext === 'png' ? 'image/png' : 'image/jpeg';
  const path = `clubs/${clubId}/logo.${asset.ext}`;
...
export async function pickAndUploadClubAvatar(clubId: string): Promise<string | null> {
  ...
  return uploadClubAvatarAsset(clubId, asset);
}
